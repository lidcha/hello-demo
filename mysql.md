...
1.  Mysql为什么用B+树，B+树和B树，红黑树等有什么区别
    ...
    假设主键类型BIGINT 占 8B，指针占 6B，算上额外开销，每个内部节点大约能存 16 * 1024 / 14 ≈ 1200 个 key。
...
4.  Mysql索引的数据结构和原理，索引优化，索引失效的条件，索引下推
    ...
    索引下推不一定是索引失效时才会生效，重点应该放在减少回表次数，例如以A、B两列建索引，查询条件为where A = 1 and B = 3，如果没有索引下推，那需要先查找到A = 1的记录范围，然后把对应的主键取出后，再回表查询这些记录并判断 B = 3是否成立。而启用了索引下推后，那在存储引擎层就会把不满足 B = 3 的条件过滤掉，减少回表的记录数。
...
12. 深分页优化方案
    ...
    **深分页**是指查询偏移量过大的场景，称为深度分页，会导致查询性能较低。例如：
    ```
