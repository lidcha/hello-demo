...
4.  Mysql索引的数据结构和原理，索引优化，索引失效的条件，索引下推
    ...
    3. 查询时避免索引失效，例如查询条件避免函数使用，避免子查询，避免非前缀匹配，避免在索引列出现NULL值等
...
7.  redo log, undo log, bin log的原理和作用
    ...
    **redo log（重做日志）**
    ...
    - Redo log的写入流程： 
        ...
        - 当脏页达到阈值或者事务提交时，InnoDB会将脏页数据写入到数据文件，但是刷盘前必须确保该页对应的redolog已罗盘，否则数据崩溃回复数据时会丢失数据。
...
16. 慢SQL怎么优化
    ...
    - 其他优化手段例如：避免SELECT * , 使用子查询来代替JOIN，将复杂查询分解为多个简单子查询。
