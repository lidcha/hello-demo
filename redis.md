...
1.  Redis为什么快。Redis是否单线程（单IO线程，新版本支持多线程）
    ...
    **AE事件**
    Redis为了跨平台处理I/O，自己写了个事件驱动框架，叫ae（asynchronous events），它是Redis的事件循环，主要分两类：文件事件和时间事件。
    - 文件事件：对应socket套接字的读写，底层依赖操作系统的多路复用机制，一个文件事件绑定一个回调函数，例如AE_READABLE--> 新连接到来/socket有数据可读；Redis不自己轮询，而是把socket注册内核，由内核负责监控。
    - 时间事件：定时任务例如定期清理过期键、AOF刷盘等，Redis自己维护了一个时间事件链表，每次事件循环时检查有没有该执行的任务。

    事件驱动机制：aeEventLoop里用一个无限循环，先处理所有已经触发的文件事件I/O，再处理时间事件，如果都没事，就调用epoll_wait阻塞等待新的事件。总结起来就是Redis中的ae事件循环就是“单线程 + I/O多路复用 + 回调分发”的高效的事件驱动系统。

    1. Redis 不自己轮询，而是把 socket 注册到 epoll/kqueue/select，由内核负责监控。

    2. I/O 事件触发 → 调用相应的 文件事件回调（比如 accept 新连接、读客户端命令、写响应）。时间事件触发 → 调用 时间事件回调（比如 serverCron、AOF刷盘）。

    3. 整个过程都在一个线程里串行完成，但因为 epoll/kqueue 是多路复用机制，所以能同时处理成千上万个连接。
        
        ![redis事件驱动时序图](image-18.png)
...
2.  Redis的八种基本数据结构，原理和使用场景
    ...
    - 有序集合（ZSet）：Redis中zSet的底层实现有两种，当数据量小的时候，用ziplist压缩链表（本质是一个字节数组）存储，节省内存；当数据量大时采用跳表skiplist+hash表的组合。skiplist实现按score有序存储，hash表负责member -> score的映射，保证快速查找。跳表是通过多层有序单链表来实现快速查找，同一层的foward指针指向本层的下一个节点，节点内部通过维护level数组实现逻辑分层的效果，查找时从最高层级查，最高层仅包含少量的几个元素，最底层包含所有的元素，快速跳过不需要的元素。

	Redis中选择使用跳表的原因是基于性能、内存、复杂度的平衡。总结来说就是实现简单（不需要维护树的平衡操作逻辑）；范围遍历（范围遍历快；不像树的范围遍历复杂）；
	
    Redis中跳表的实现主要由两个核心的数据结构：1. 跳表节点（存储对象、score、前进后退指针以及leve数组层索引） 2. 跳表结构（包含了头尾节点）
	每次查找时，判断下一个节点是否大于目标，是的话，则将索引数组减一，进入下一层，继续向右查找。

    **Redis为什么要自定义字符串的数据结构？**
    - 首先主要是 C 语言中的原生字符串结构不靠谱，需要依赖'\0'来判断结尾，会导致长度获取时间复杂度O(n)；不安全例如如果字符串有中间的'\0'就会被截断；空间扩展时可能会频繁realloc。
    - 因此Redis自定义了SDS，增加len字段直接保存长度，避免获取长度时需要遍历；由于不用使用'\0'做结尾，因此字符串中可以存任意的数据，包括中间有'\0'的情况；还做了空间预分配，按一定规则扩容。
    - 总得来讲自定义SDS是基于性能安全和灵活性各方面考虑
...
4. Redission有了解过吗？redissoin的时间轮？
    ...
    Redission是基于Redis的Java分布式框架，不仅是一个Redis客户端，而且还封装了很多常用的分布式组件，例如分布式锁、分布式集合、分布式队列、布隆过滤器等。

    在redisson中，时间轮（Time wheel）主要被用来做延迟任务调度，从原理上看，时间轮是一种高效的定时器实现结构，适合管理大量的定时任务。可以像一个钟表盘：
    - 每个格子代表一个时间槽
    - 把需要延迟执行的任务放到对应的槽里
    - 表盘指针按固定步长转动，当指针走到某个槽时，会触发其里面的任务。

    优点在于，不需要频繁调整优先队列，只需要简单的数组+指针+链表即可高效维护大批量的延迟任务，复杂度接近O(1);

    运行过程：
    1. 新任务进来时，根据延迟时间计算出它应该落在哪个槽
    2. 每次tick（间隔）指针就走到下一个槽
    3. 当指针走到某个槽，就检查这个槽里的任务，到期了就执行，没到期就重新挂到未来的槽中。

    Redisson中的分布式锁，也是Redis + 基于lua脚本 + 看门狗线程实现的

    利用lua脚本在redis中的原子性，做加锁、释放锁的操作；同时会开启一个看门狗线程，针对这个锁，每隔一段时间，自动给锁续期，这样锁的过期时间就和业务线程的业务耗时绑定，只要业务还在执行，锁就一直存在，不会说过期被其他线程获取

    优势是：可重入、自动续期、还有更高级的公平锁、读写锁等高级锁功能。